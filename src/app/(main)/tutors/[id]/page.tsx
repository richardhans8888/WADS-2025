        ...
        <div className="lg:col-span-8 space-y-8">
              ...
              <div className="flex-1 space-y-4">
                <div className="flex flex-wrap justify-between items-start gap-4">
                  <div>
                    ...
                    <p className="text-gray-400 text-lg font-medium">{tutor.title}</p>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 pt-2">
                  {tutor.tags.map((tag) => (
                    <span key={tag} className="bg-[#1E2330] text-gray-300 text-sm font-medium px-4 py-1.5 rounded-full border border-gray-800">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-[#151921] rounded-2xl p-6 border border-gray-800 text-center hover:border-blue-500/30 transition-colors group">
              ...
              <div className="text-2xl font-bold text-white mb-1">{tutor.stats.students}</div>
              <div className="text-xs text-gray-500 uppercase tracking-wider font-bold">Students Taught</div>
            </div>
            <div className="bg-[#151921] rounded-2xl p-6 border border-gray-800 text-center hover:border-yellow-500/30 transition-colors group">
              ...
              <div className="text-2xl font-bold text-white mb-1">{tutor.stats.rating}</div>
              <div className="text-xs text-gray-500 uppercase tracking-wider font-bold">Average Rating</div>
            </div>
            <div className="bg-[#151921] rounded-2xl p-6 border border-gray-800 text-center hover:border-emerald-500/30 transition-colors group">
              ...
              <div className="text-2xl font-bold text-white mb-1">{tutor.stats.successRate}</div>
            ...
            <div className="bg-[#151921] rounded-2xl p-6 border border-gray-800 text-center hover:border-purple-500/30 transition-colors group">
              ...
              <div className="text-2xl font-bold text-white mb-1">{tutor.stats.experience}</div>
              <div className="text-xs text-gray-500 uppercase tracking-wider font-bold">Experience</div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="grid gap-6">
              {tutor.reviews.map((review) => (
                <div key={review.id} className="bg-[#151921] rounded-3xl p-8 border border-gray-800 flex gap-6 hover:bg-[#1A202C] transition-colors">
                  <div className={`w-14 h-14 rounded-full ${review.color} flex items-center justify-center text-white font-bold text-xl shrink-0`}>
                    {review.avatar}
                  </div>
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <span className="text-sm text-gray-500">{review.course}</span>
                      </div>
        ...
        <div className="lg:col-span-4 space-y-8">
          ...
          <div className="bg-[#151921] rounded-3xl p-6 border border-gray-800 sticky top-24">
            <div className="flex justify-between items-start mb-8">
              <div>
                <span className="text-sm text-gray-500 font-medium block mb-1">Hourly Rate</span>
                <div className="flex items-baseline gap-1">
                  <span className="text-4xl font-bold text-white">${tutor.rate}</span>
          ...
          <div className="bg-[#151921] rounded-3xl p-6 border border-gray-800">
            <div className="flex justify-between items-center mb-6">
              ...
              <span className="text-xs text-gray-500 font-medium">Oct 2023</span>
            </div>

            <div className="grid grid-cols-7 gap-2 mb-8 text-center">
              {dates.map((d, idx) => (
                <div key={idx} className="space-y-2">
                  <div className="text-[10px] text-gray-500 font-medium uppercase">{d.day}</div>
                  <button 
            ...
            <div className="space-y-3">
              <span className="text-xs text-gray-500 font-medium block mb-2">Available slots on Oct {activeDate}nd:</span>
