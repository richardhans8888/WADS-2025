...
export default function MySchedulePage() {
  ...
  const [events, setEvents] = useState<EventItem[]>([
    { id: "1", date: toISO(current.year, current.month, 10), title: "Calculus II Study", time: "4:00 PM", color: "bg-blue-500" },
    { id: "2", date: toISO(current.year, current.month, 12), title: "Physics Review", time: "10:00 AM", color: "bg-emerald-500" },
    { id: "3", date: toISO(current.year, current.month, 17), title: "Project Meeting", time: "2:30 PM", color: "bg-purple-500" },
  ]);
  ...
  const monthLabel = useMemo(() => new Date(current.year, current.month, 1).toLocaleString(undefined, { month: "long", year: "numeric" }), [current]);
  ...
  function saveEvent() {
    ...
    setEvents((prev) => [...prev, { id, date: draftDate, title: title.trim(), time: time.trim(), color }]);
    setOpen(false);
  }
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button onClick={() => openNew(toISO(current.year, current.month, new Date().getDate()))}>
            ...
            return (
              <button
                ...
                className={`h-28 rounded-xl border p-2 text-left transition-colors ${
                  inCurrent ? "bg-white dark:bg-[#0F1117] border-gray-200 dark:border-gray-800 hover:border-blue-500" : "bg-gray-100 dark:bg-[#0D1320] border-gray-200 dark:border-gray-800 opacity-70"
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className={`text-xs font-medium ${inCurrent ? "text-gray-700 dark:text-gray-300" : "text-gray-400 dark:text-gray-500"}`}>
                    {cell.day}
                  </div>
                  <div className="flex gap-1">
                    {cellEvents.slice(0, 3).map((e) => (
                      <span key={e.id} className={`inline-block w-2 h-2 rounded-full ${e.color}`} />
                    ))}
                  </div>
                </div>
                <div className="mt-2 space-y-1">
                  {cellEvents.slice(0, 3).map((e) => (
                    <div key={e.id} className={`text-[11px] px-2 py-1 rounded ${e.color} text-white`}>
                      {e.title} {e.time ? `• ${e.time}` : ""}
                    </div>
                  ))}
                  {cellEvents.length > 3 && (
                    <div className="text-[11px] text-gray-500 dark:text-gray-400">+{cellEvents.length - 3} more</div>
                  )}
                </div>
              </button>
            );
              ...
              <div key={e.id} className="flex items-center justify-between bg-white dark:bg-[#0F1117] border border-gray-200 dark:border-gray-800 rounded-xl p-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500 dark:text-gray-400">{new Date(e.date).toLocaleDateString()} {e.time}</div>
                  </div>
                </div>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="bg-white dark:bg-[#0F1117] border border-gray-200 dark:border-gray-800">
            ...
            <div className="space-y-3">
              <div className="text-sm text-gray-600 dark:text-gray-400">{draftDate ? new Date(draftDate).toDateString() : ""}</div>
              <input
                placeholder="Title"
                value={title}
              ...
              />
              <div className="flex items-center gap-2">
                {["bg-blue-500", "bg-emerald-500", "bg-purple-500", "bg-amber-500", "bg-rose-500"].map((c) => (
                  <button
                    key={c}
                    onClick={() => setColor(c)}
