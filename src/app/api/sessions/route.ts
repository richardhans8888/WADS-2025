...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await supabaseAdmin
    .from('chat_sessions')
    .insert({ user_id: userId, title: title || 'New Session', subject: subject || 'General' })
    .select()
    .single();
