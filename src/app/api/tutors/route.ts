...
export async function POST(req: NextRequest) {
  ...
  if (!userId || !subjects) return NextResponse.json({ error: 'userId and subjects required' }, { status: 400 });
  ...
  const { data, error } = await supabaseAdmin
    .from('tutors')
    .insert({ user_id: userId, subjects, hourly_rate: hourlyRate, bio, availability })
    .select()
    .single();
  ...
  await supabaseAdmin.from('user_profiles').update({ role: 'tutor' }).eq('id', userId);
