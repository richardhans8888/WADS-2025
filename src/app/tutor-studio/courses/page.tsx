    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-400 text-sm">Manage your ongoing classes and cohorts.</p>
        </div>
        <Button className="bg-teal-500 hover:bg-teal-600 text-black font-bold" onClick={() => setOpen(true)}>
        ...
        {activeCourses.map((course) => (
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <div className={`p-3 rounded-xl ${course.color}/10 text-${course.color.replace('bg-', '')}`}>
            ...
            <div className="mt-6 flex items-center text-sm font-medium text-gray-400 group-hover:text-white transition-colors">
              Manage Course <ArrowRight className="w-4 h-4 ml-2 group-hover:translate-x-1 transition-transform" />
            </div>
          </Link>
        ))}

        {/* Add New Course Placeholder Card */}
        <button onClick={() => setOpen(true)} className="flex flex-col items-center justify-center h-full min-h-[250px] rounded-2xl border-2 border-dashed border-gray-800 hover:border-teal-500/50 hover:bg-teal-500/5 transition-all group">
          ...
          <span className="font-medium text-gray-400 group-hover:text-teal-400">Add New Course</span>
        </button>
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="bg-[#0F1117] border-gray-800">
          <DialogHeader>
            ...
            <DialogDescription>Fill out the details and submit for review.</DialogDescription>
          </DialogHeader>
          <div className="space-y-3 mt-2">
            <div>
              <label className="text-xs text-gray-400">Course Title</label>
              <input className="w-full mt-1 bg-[#15181E] border border-gray-800 rounded-lg px-3 py-2 text-sm text-white" placeholder="e.g., Data Structures" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs text-gray-400">Category</label>
                <input className="w-full mt-1 bg-[#15181E] border border-gray-800 rounded-lg px-3 py-2 text-sm text-white" placeholder="Computer Science" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea className="w-full mt-1 bg-[#15181E] border border-gray-800 rounded-lg px-3 py-2 text-sm text-white" rows={4} placeholder="Briefly describe course outcomes, structure and requirements." />
            </div>
